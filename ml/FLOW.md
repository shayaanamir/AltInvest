...
**Model**: RandomForest (n=300, max_depth=8, 5-fold CV accuracy: 83%)

**Labeling strategy** (percentile-based, adapts to any asset):
```
composite_risk =
    0.40 × |volatility_14d|   (dominant driver)
  + 0.25 × |return_1d|        (short-term shock)
  + 0.20 × |return_7d|        (weekly swing)
  + 0.15 × |return_30d|       (monthly trend)
...
**API inverts it**: `contract_score = 100 - risk_score` (higher = safer for user display)
...
| Percentile-based risk labels | Adapts to any asset's price distribution automatically |
